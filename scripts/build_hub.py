...
def page(title, description, body, route='/', active=None, asset=None):
    ...
    search_match = re.search(r'<form id="library-search".*?</form>', body, re.S)
    header_search = search_match.group(0) if search_match else ''
    if header_search: body = body.replace(header_search, '', 1)
    library_heading = '<a class="header-library-link" href="/" aria-label="Asset library" title="Asset library"><span class="control-icon" aria-hidden="true">▦</span><span class="control-label">Asset library</span></a>'
    ...
    viewer_header = '<div class="viewer-toolbar" id="viewer-toolbar" hidden><button type="button" id="viewer-back" class="text-button" aria-label="Back to library" title="Back to library">←</button><span class="viewer-asset-mark" id="viewer-asset-mark" aria-hidden="true">▧</span><h2 id="viewer-title" tabindex="-1"></h2></div>' if filter_sidebar else ''
    current_asset = asset['id'] if asset else ''
    contact_url = '/hub/contact/' + ('?asset='+current_asset if current_asset else '')
    trigger = f'<a class="conversation-trigger" id="conversation-trigger" href="{contact_url}" data-current-asset="{current_asset}" aria-haspopup="dialog" aria-controls="conversation-panel" aria-label="Start a conversation" title="Start a conversation"><span class="control-icon" aria-hidden="true">✉</span><span class="control-label">Start a conversation</span></a>'
    ...
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
      ...
      <meta name="theme-color" content="#f5f4ec"><link rel="stylesheet" href="/css/command-center.css"><script src="/js/command-center.js" defer></script><script src="/js/asset-viewer.js?v=20260909-live-source" defer></script><link rel="alternate" type="application/rss+xml" title="Autumn Memo" href="/index.xml"></head>
      <body data-filter-layout="{'rail' if filter_sidebar else 'none'}"><a class="skip" href="#main">Skip to content</a><div class="app-shell"><div class="workspace"><header class="topbar library-topbar">{library_heading}{header_search}{viewer_header}<div class="topbar-links"><a id="about-trigger" class="about-trigger" href="/about/" aria-haspopup="dialog" aria-controls="about-panel" aria-label="About Author" title="About Author"><span class="control-icon" aria-hidden="true">ⓘ</span><span class="control-label">About Author</span></a>{trigger}<button type="button" id="header-collapse" class="header-collapse" aria-expanded="true" aria-label="Collapse header to icons" title="Collapse header to icons">⌃</button></div></header>
...
def build_pages():
    intro = ''
    controls = '''<form id="library-search" class="compact-search" role="search" action="/"><label class="sr-only" for="asset-search">Search the collection</label><input id="asset-search" name="q" type="search" placeholder="Search the collection…" autocomplete="off"><input id="asset-kind" type="hidden" name="kind"><input id="asset-stream" type="hidden" name="stream"></form>'''
