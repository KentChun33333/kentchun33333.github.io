...
def build_pages():
    intro = '<div class="collection-heading"><h1>Asset library</h1></div>'
    controls = '''<form id="library-search" class="compact-search" role="search" action="/"><label class="sr-only" for="asset-search">Search the collection</label><input id="asset-search" name="q" type="search" placeholder="Search research, demos, skills, writing…" autocomplete="off"><input id="asset-kind" type="hidden" name="kind"><input id="asset-stream" type="hidden" name="stream"><a id="filters-trigger" class="filter-trigger" href="#filters-panel" aria-haspopup="dialog" aria-controls="filters-panel">Filters <span id="filter-count"></span></a></form>'''
    types=''.join(f'<button type="button" data-kind-shortcut="{kind}" aria-pressed="false">{label}</button>' for kind,label in [('', 'All assets'),('Research','Research'),('Demo','Demos'),('Skill','Skills'),('Article','Writing & blogs'),('Tool','Tools'),('Protocol','Protocols')])
    ...
    filters='<dialog id="filters-panel" class="conversation-panel filters-panel" aria-labelledby="filters-title"><div class="conversation-heading"><h2 id="filters-title" tabindex="-1">Filter the collection</h2><button type="button" id="filters-close" aria-label="Close filters">×</button></div><fieldset class="filter-group"><legend>Asset type</legend><div class="filter-options">'+types+'</div></fieldset><fieldset class="filter-group"><legend>Workstream</legend><div class="filter-options">'+streams+'</div></fieldset><p id="filter-results" role="status"></p><div class="filter-panel-actions"><button type="button" id="filters-apply" class="button">Show results</button><button type="button" id="filters-reset" class="text-button">Reset all</button></div></dialog>'
