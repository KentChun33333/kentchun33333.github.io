...
def build_pages():
    intro = head('KENT CHIU · RESEARCH, ENGINEERING & LIFE', 'An open collection of work & ideas.', 'Explore my research, interactive systems, reusable skills, and personal writing. Open an asset, follow its connections, or get in touch about it.')
    about = '''<details class="library-about" id="about"><summary>About Kent & my experience</summary><div><p>I work across AI systems, research, and engineering. Autumn Memo began during my research exchange in Randall Davis’s MIT-CSAIL lab in 2016.</p><p>Explore agent workflow design, evaluation research, and product engineering through the artifacts below, alongside personal stories and illustrated notes.</p><div class="about-links"><a href="/hub/assets/ter/">Agent workflows ↗</a><a href="/hub/assets/gse/">Evaluation research ↗</a><a href="/hub/assets/publisher/">Product engineering ↗</a><a href="/about/">Original background & photos ↗</a></div></div></details>'''
    controls = f'''<form class="library-controls" role="search" action="/"><label class="search-label">Search the collection<input id="asset-search" name="q" type="search" placeholder="Search memory, MIT, buying a home…" autocomplete="off"></label><label>Asset type<select id="asset-kind" name="kind"><option value="">All types</option>{''.join(f'<option value="{E(k)}">{E("Writing & blogs" if k=="Article" else k)}</option>' for k in sorted({a['kind'] for a in ASSETS}))}</select></label><label>Workstream<select id="asset-stream" name="stream"><option value="">All streams</option><option>Build</option><option>Research</option><option>Publish</option></select></label><button type="reset" class="text-button">Reset</button></form>'''
    filters='<div class="asset-type-links" aria-label="Browse by asset type">'+''.join(f'<a href="/?kind={kind}" data-kind-shortcut="{kind}">{label}</a>' for kind,label in [('', 'All assets'),('Research','Research'),('Demo','Demos'),('Skill','Skills'),('Article','Writing & blogs'),('Tool','Tools'),('Protocol','Protocols')])+'</div>'
    ...
    library=intro+about+controls+filters+f'<p id="result-count" role="status">{len(ASSETS)} assets</p><div id="asset-results" class="card-grid">'+''.join(card(a,True) for a in ordered)+'</div><div id="empty-results" class="empty-state" hidden><h2>No matching assets</h2><p>Try a broader search or reset your filters.</p></div><noscript><p>All assets are listed here. Interactive filtering requires JavaScript.</p></noscript>'
