...
def page(title, description, body, route='/', active=None, asset=None):
    ...
    viewer_header = '<div class="viewer-toolbar" id="viewer-toolbar" hidden><button type="button" id="viewer-back" class="text-button" aria-label="Back to library" title="Back to library">←</button><span class="viewer-asset-mark" id="viewer-asset-mark" aria-hidden="true">▧</span><h2 id="viewer-title" tabindex="-1"></h2><button type="button" id="viewer-share" class="text-button" aria-label="Copy asset link" title="Copy asset link"><span aria-hidden="true">⤴</span><span class="control-label"> Share</span></button></div>' if filter_sidebar else ''
    ...
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
      ...
      <body data-filter-layout="{'rail' if filter_sidebar else 'none'}"><a class="skip" href="#main">Skip to content</a><div class="app-shell"><div class="workspace"><header class="topbar library-topbar">{library_heading}{viewer_header}<div class="topbar-links"><a id="about-trigger" class="about-trigger" href="/about/" aria-haspopup="dialog" aria-controls="about-panel" aria-label="About and experience" title="About and experience"><span class="control-icon" aria-hidden="true">ⓘ</span><span class="control-label">About & experience</span></a>{trigger}<button type="button" id="header-collapse" class="header-collapse" aria-expanded="true" aria-label="Collapse header to icons" title="Collapse header to icons">⌃</button></div></header>
