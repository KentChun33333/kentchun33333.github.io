...
def build_pages():
    intro = '<div class="collection-heading"><h1>Asset library</h1><p>Research, systems, skills, and stories — collected in one place.</p></div>'
    controls = '''<form id="library-search" class="compact-search" role="search" action="/"><a id="filters-trigger" class="filter-trigger" href="#filters-panel" aria-haspopup="dialog" aria-controls="filters-panel">Asset types <span aria-hidden="true">☰</span><span id="filter-count"></span></a><label class="sr-only" for="asset-search">Search the collection</label><input id="asset-search" name="q" type="search" placeholder="Search research, demos, skills, writing…" autocomplete="off"><input id="asset-kind" type="hidden" name="kind"><input id="asset-stream" type="hidden" name="stream"></form>'''
    types=''.join(f'<button type="button" data-kind-shortcut="{kind}" aria-pressed="false"><span>{label}</span><span class="type-total">{sum(1 for a in ASSETS if not kind or a["kind"]==kind)}</span></button>' for kind,label in [('', 'All assets'),('Research','Research'),('Demo','Demos'),('Skill','Skills'),('Article','Writing & blogs'),('Tool','Tools'),('Protocol','Protocols')])
