...
The homepage is the asset library with a compact title and one search bar. About & experience opens a right drawer from the top bar; an Asset types button opens a non-modal left sidebar with asset-type and workstream buttons. The sidebar pushes the main page narrower and lets its card grid reflow; the collection remains interactive and scrollable. There is no repeated type dropdown or large introductory block. Research, demos, skills, protocols, tools, and writing are asset types in one collection. A top-bar Start a conversation action opens a right-side modal drawer. Asset pages preselect the current asset. Individual contact buttons and the left navigation panel are removed; configured available offers retain their buy actions. The knowledge graph remains in the asset layer, with its existing URL retained for direct access.
