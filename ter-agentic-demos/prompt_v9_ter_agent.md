...
In Frame 4, add a prominent three-step conclusion path: `Selected target → Open-web deep dive → Validated risk`. Redraw the evidence network as a directional chain that joins the CORE-KYC stationery baseline and copy-paper invoice to the customer, follows the USD 301,760 payment to Aster Peak FZE, and then highlights two converging validations: shipping/customs identifies computer-controlled metal-cutting machinery, while the External Web Agent independently confirms Aster Peak's industrial-machinery business. Use distinct colors and an explicit legend for baseline evidence, payment flow, web validation and the goods conflict.
