...
- Every Page 2 content section is independently foldable: Trigger Context, Profile, Key Insight Extraction, OpenWeb Review and Analyst Feedback. Extraction sections remain open initially while Trigger Context remains collapsed.
- Page 3 places `Next Step` inside the Refinement drawer as a nested foldable section. It provides two human choices: `Done`, which returns to the queue with the review complete, and `Further Investigation`, which runs the existing additional OpenWeb investigation flow.
