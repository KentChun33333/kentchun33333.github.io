# Prompt v11 — Explainable Partial-to-Full Exit Escalation

## Objective

Create `ter-bank-multi-agent-flow-v11.html` as a standalone evolution of v10. Preserve v10 unchanged and retain the current visual UI, fictional Orchid Meridian Trading case, BANK styling, three-page analyst journey, seven evidence sources, recommendation placement above Refinement, appendix reports, reset, export, autoplay, and compact icon-only animation navigation.

V11 must make both the evidence story and the change in exit decision immediately understandable. The left panel remains a compact workforce roster. The right panel progressively assembles structured results, while Page 3 clearly explains why the initial outcome is Partial Exit and what new evidence changes it to Full Exit.

Remove the `CONCEPT DEMO · FICTIONAL DATA` label from the global header. Reuse that top-right space for short, page-aware controls: Page 1 has no controls, Page 2 shows `← Back` to return to Page 1, and Page 3 shows `New`, `Replay`, and `Export`. Put the animation-level `Replay` control in the animation header at upper right, and remove the `AUTO-PLAY · MANUAL CONTROL AVAILABLE` caption. Do not duplicate these controls in page headings or the animation footer. On Page 3, label the primary refinement action `Run Refinement` and do not render a second replay control beside or beneath it.

On Page 1, remove the entire introductory block containing `Page 1 · Compact intake`, `Start with the customer and evidence bundle`, and its explanatory paragraph. Render only the two intake cards with uppercase headings `CASE IDENTITY` and `EVIDENCE UPLOAD`. The evidence list must be empty initially, with a clear no-evidence state and disabled extraction button. Add a compact `Auto` button that loads the seven prepared demo files and enables extraction; choosing or dropping local files must also populate the evidence state and enable extraction.

Remove the introductory page-heading blocks from Page 2 and Page 3 as well. Page 2 must begin directly with the evidence-review layout, and Page 3 must begin directly with the case summary strip and final report layout.
...
Preserve the progressive v11 information design and the current document animation inside the **main canvas**.
...
## V11 human-correction propagation
...
1. Prompt v11 and HTML v11 exist; v10 is unchanged.
...
38. Animation Replay sits in the animation header, and the animation footer contains only exit, previous, progress/dots and next/finish navigation.
...
41. Page 1 contains no introductory page heading and uses uppercase `CASE IDENTITY` and `EVIDENCE UPLOAD` card titles.
42. Evidence starts empty, extraction is disabled, and `Auto` loads the seven prepared demo sources before extraction can begin.
43. Page 2 and Page 3 contain no eyebrow, title or explanatory page-heading blocks; their operational content starts immediately below the workflow navigation.
